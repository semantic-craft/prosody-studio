...
enum SettingsTheme {
    ...
    // MARK: - Brand (the single accent — skin-driven via `Skin.current`, default 深大红)
    //
    // The `wine*` names are kept for call-site stability; they now resolve from the active skin's
    // accent so the whole settings surface follows the chosen skin (深大红 / 斯坦福). For the
    // default 深大红 skin these are identical to the former literals.
    ...
    // MARK: - Domain chips (low-sat, one per domain — no rainbow)

    static var cLegal: Color { wine }
    static var cLegalBg: Color { wineTint }
    static let cEng = dyn(0x2E7C74, 0x6FBDB3)
    static let cEngBg = dyn(0x2E7C74, 0x6FBDB3, lightA: 0.11, darkA: 0.20)
    static let cSys = dyn(0x8A7355, 0xC2A782)
    static let cSysBg = dyn(0x8A7355, 0xC2A782, lightA: 0.12, darkA: 0.20)
    ...
    /// Serif content face (`--serif`). Charter ships on macOS; Chinese falls back.
    static func serif(_ size: CGFloat, weight: Font.Weight = .semibold) -> Font {
        Font.custom("Charter", size: size).weight(weight)
    }

    // MARK: - Section header / footer helpers (for grouped `Form` panes)

    static func header(_ text: String) -> some View {
        Text(text).font(headerFont).foregroundStyle(ink).textCase(nil)
    }
...
extension View {
    /// Warm-paper chrome for a grouped settings `Form`: hide the native grey
    /// scroll background so the parchment window shows through, enlarge the base
    /// type, and run controls at the larger size.
    func warmForm() -> some View {
        self
            .formStyle(.grouped)
            .scrollContentBackground(.hidden)
            .listRowBackground(SettingsTheme.card)   // ivory section cards (E · 暖纸卡片化)
