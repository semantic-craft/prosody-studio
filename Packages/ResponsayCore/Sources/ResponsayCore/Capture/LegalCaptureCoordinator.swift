...
public struct LegalCaptureCoordinator {
    ...
    public func route(text: String) -> LegalPaletteOutcome? {
        ...
        return runtime.route(
            context: context, now: Date(), browserURL: context.browserURLHost,
    ...
    /// User-confirmed scene: re-suggest the scene-scoped skill palette and rebuild
    /// the outcome with a confirmed (above-threshold) classification.
    public func selectScene(_ scene: LegalScene, in outcome: LegalPaletteOutcome) -> LegalPaletteOutcome? {
        guard let runtime else { return nil }
        let cards = runtime.suggestSkills(
            scene: scene, stage: outcome.stage,
            confidence: outcome.confidence, profile: profileProvider?(),
            enabled: enabledProvider?())
        let confirmed = SceneStageClassification(
            scene: scene, stage: outcome.stage,
            confidence: max(outcome.confidence, LegalSceneStageRouter.autoThreshold),
            reasons: ["用户已确认场景：\(scene.rawValue)"], shouldAskUser: false)
        return LegalPaletteOutcome(
            decision: .autoCandidates(confirmed, candidates: cards), cards: cards)
    }
