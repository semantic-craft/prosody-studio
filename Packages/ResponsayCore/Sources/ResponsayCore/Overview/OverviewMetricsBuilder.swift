...
public struct OverviewMetricsBuilder: Sendable {
    ...
    /// The character total for one item — the idiomatic text if present, else the
    /// raw source (matches the History screen's 字数 convention).
    static func characterCount(_ item: CaptureItem) -> Int {
        (item.idiomatic.isEmpty ? item.sourceText : item.idiomatic).count
    }
