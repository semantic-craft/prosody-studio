...
public enum LLMSearchResultParser {
    ...
    private static func parseAnnotations(_ message: [String: Any], providerId: String) -> SearchResult? {
        guard let annotations = message["annotations"] as? [[String: Any]],
              let first = annotations.first(where: { ($0["url"] as? String)?.isEmpty == false }) else {
            return nil
        }
        let title = first["title"] as? String ?? ""
        let url = first["url"] as? String ?? ""
        let snippet = first["summary"] as? String ?? first["content"] as? String ?? ""
