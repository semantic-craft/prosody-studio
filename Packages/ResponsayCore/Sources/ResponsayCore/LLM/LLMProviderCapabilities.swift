...
public struct LLMProviderCapabilities: Sendable, Equatable {
    ...
    private enum Channel { case qwen, mimo, zhipu, gemini, openai, openrouter, ollama, otherKnown, unknown }
    ...
    private static func channel(providerId: String, host: String) -> Channel {
        ...
        case "deepseek", "minimax", "kimi": return .otherKnown
        case "custom": break
        default: break
        }
        ...
        if h.contains("deepseek") || h.contains("minimax") || h.contains("moonshot") { return .otherKnown }
        return .unknown
    }
}
...
public struct LLMGenerationProfile: Sendable, Equatable {
    ...
    public static func resolve(
        ...
        if pid == "mimo" || host.contains("xiaomimimo") {
            return .init(
                temperature: 1.0,
                topP: 0.95,
        ...
        case .ask, .prosody, .connectivity, .polish, .rewrite:
            return .init(temperature: 0.2, topP: 0.8, maxCompletionTokens: nil, timeout: 60, thinkingDefault: false)
        }
    }
