...
public enum LLMThinkingControl {
    ...
    static func extraBody(
        ...
        case .geminiCompat:
            // Gemini's OpenAI-compat layer accepts `reasoning_effort`, with "none" disabling
            // thinking on 2.5-flash-class models (Pro / 3 models can't fully disable — model ceiling).
            let isProOr3 = model.contains("-pro") || model.contains("gemini-3")
            if !enabled && isProOr3 {
                return [:]
            }
    ...
    /// `false` (e.g. Kimi / Doubao) means the toggle is a no-op for that provider — the
    ...
    enum Channel { case openAIReasoning, dashScope, deepSeek, miniMax, openRouter, geminiCompat, mimo, zhipu, ollama, none }
