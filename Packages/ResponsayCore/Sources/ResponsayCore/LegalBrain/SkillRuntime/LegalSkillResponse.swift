...
//
// concept_map / risk_matrix (declared in `LegalOutputCardType`) map to `.fallbackText`
// in v0.1 per spec §7; they get their own cases in a later phase.
...
public enum LegalOutputCard: Codable, Sendable {
    ...
    // Custom externally-tagged Codable: `{"<caseName>": <payload>}` — a clean shape an LLM
    // can actually produce. (Swift's synthesized enum Codable wraps the payload in a `_0`
    // key, which no model emits — that mismatch forced every real reply to fallback.)
    private enum CodingKeys: String, CodingKey {
        ...
        case legalAnalysis, strategyRecommendation
    }
