# 事实查验 — verification.fact_check.cn （v1.0）

本技能用于从用户选中的文本中，精准提取出高度特征化的"法律引用"（如法条名称、案号）或"长文本事实描述"，并将其转化为结构化的 `verificationAnchors`。客户端将接收这些锚点，为用户生成各大公开法律平台（如裁判文书网、北大法宝、得理等）的溯源深链接。大模型禁止直接编造验证结果。
...
```legal-skill
{
  ...
  "language": "zh",
  "triggers": {
    "keywords": ["查验", "核实", "验真", "真的假的", "打假", "法条查验", "案号查验"],
