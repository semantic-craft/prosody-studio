...
public struct SearchStrategyGenerator: Sendable {
    ...
    /// 知网专业检索式：把检索词按空白拆成主题词，逐个 `SU=('词')` 并 AND。
    private func cnkiCard(for anchor: VerificationAnchor) -> CNKIQueryCard {
        ...
        // CNKI 专业检索无转义语法——词内单引号（Law's Empire / Hart's…，法学英文
        // 文献常见）会撕裂 SU=('…') 的引号配对，剥除最稳（猎虫④ F6）。
        func su(_ term: String) -> String {
            "SU=('\(term.replacingOccurrences(of: "'", with: ""))')"
        }
        let terms = raw.split(whereSeparator: { $0 == " " || $0 == "　" }).map(String.init).filter { !$0.isEmpty }
        let expert = terms.isEmpty
            ? su(raw.isEmpty ? anchor.label : raw)
            : terms.map(su).joined(separator: " AND ")
        return CNKIQueryCard(title: "知网专业检索式", expertQuery: expert, plainQuery: raw.isEmpty ? anchor.label : raw)
    }
}
