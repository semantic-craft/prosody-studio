...
public final class CloudQwenSpeechCaptureService: SpeechCaptureService {
    ...
    private let client: any TranscriptionAPI
    ...
    /// OpenAI/MiMo/Zhipu/custom clients (猎虫① H9).
    public init(
        provider: String,
        requireMicPermission: @escaping () throws -> Void,
        clientBuilder: (_ profile: @escaping @Sendable () -> SpeechCaptureProfile) -> any TranscriptionAPI
    ) {
    ...
    public func start(locale: CaptureLocale) throws {
        ...
        #if os(macOS)
        AudioInputDeviceSelector.apply(to: engine)   // 首选麦克风 (system default when unset)
        #endif
    ...
    /// Per-provider segment budget: GLM caps *duration* at 30 s per request, so
    /// the byte budget alone (sized for Qwen's 10 MB-base64 limit, ≈187 s) would
    /// hard-fail every long clip routed to Zhipu.
    nonisolated static func segmentBudget(forProvider provider: String) -> Int {
        provider == "zhipu"
            ? PCMWAVSegmenter.glmMaxSegmentBytes
            : PCMWAVSegmenter.defaultMaxSegmentBytes
    }
    ...
    private func transcribeSegmented(_ fileURL: URL) async throws -> String {
        ...
        let segments = PCMWAVSegmenter.segmentedWAVs(
            samples: samples,
            maxSegmentBytes: Self.segmentBudget(forProvider: providerName))
    ...
    private func transcribeSegment(
        ...
        if let streamingClient = client as? any StreamingTranscriptionAPI {
            var segmentText = ""
