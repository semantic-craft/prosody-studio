...
public final class AppleSpeechCaptureService: SpeechCaptureService {
    ...
    public func start(locale: CaptureLocale) throws {
        ...
        // The tap fires on the audio I/O thread and the recognition handler on a
        // background queue. Both are @Sendable (so they run nonisolated, NOT pinned
        // to the main actor) and touch only Sendable values (`session`, `levelCont`) —
        // never `self`.
