...
@Test @MainActor func hold_expressFails_setsError_savesNothing() async throws {
...
@Test @MainActor func teaching_analyzeFails_keepsExpressResultWithNeedsNetworkNote() async throws {
    let (vm, store) = makeTeachingVM(
        transcript: "我看看",
        coach: MockCoachAPI(
            result: ExpressionResult(idiomatic: "Let me check.", original: "我看看", reasons: ["更口语"]),
            analyzeError: .message("offline: prosody needs network")))

    vm.push(outputMode: .teachingFeedback)
    await vm.release()

    // Express survived; we did NOT fall into the error phase.
    #expect(vm.phase == .review)
    #expect(vm.result?.idiomatic == "Let me check.")
    #expect(vm.result?.reasons.contains { $0.contains("联网后看韵律") } == true)
    // The useful coaching was still saved.
...
@Test @MainActor func translatePreview_rendersCardWithoutTouchingHost() async throws {
    ...
    let vm = QuickCaptureViewModel(
        ...
        streamingInsert: { _, _, _ in streamingCalled = true; return .streamed(text: "streamed") })

    await vm.processText("截图文字", outputMode: .translatePreview)

    #expect(inserter.inserted.isEmpty)        // host untouched
    #expect(streamingCalled == false)         // streaming direct-write bypassed too
...
@Test @MainActor func rawStreaming_previewsPartialsAndInsertsFinalOnce() async throws {
    let speech = PartialMockSpeechCaptureService()
    let inserter = MockTextInserter()
    var streamed: [(delete: Int, text: String)] = []
    let vm = QuickCaptureViewModel(
        speech: speech,
        coach: MockCoachAPI(),
        store: FileCaptureStore(fileURL: FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString)),
        inserter: inserter,
        streamingASRInsert: { deleteCount, newText in
            streamed.append((deleteCount, newText)); return true
        })
...
@Test @MainActor func rawStreaming_ignoresHostStreamingHookAndAvoidsCleanupPath() async throws {
    let speech = PartialMockSpeechCaptureService()
    let inserter = MockTextInserter()
    var streamed: [(delete: Int, text: String)] = []
    let vm = QuickCaptureViewModel(
        ...
        inserter: inserter,
        streamingASRInsert: { deleteCount, newText in
            streamed.append((deleteCount, newText))
            return newText.isEmpty ? false : true
        })
    ...
    speech.transcriptToReturn = "今天测试 Qwen3-ASR"
    await vm.toggle(outputMode: .rawTranscript)
...
// （StreamingInsertSettings scope note：只有事务性插入走直写）。
