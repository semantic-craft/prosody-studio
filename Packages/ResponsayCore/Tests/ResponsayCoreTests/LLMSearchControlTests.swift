...
struct LLMSearchControlTests {
    ...
    @Test func kimi_supportsSearch() {
        #expect(LLMSearchControl.supportsSearch(providerId: "kimi", baseURLHost: "api.moonshot.cn"))
    }
    ...
    @Test func kimi_mimo_zhipu_supportSourceResults() {
        #expect(LLMSearchControl.supportsSourceResults(providerId: "kimi", baseURLHost: "api.moonshot.cn"))
    ...
    @Test func searchDisabled_returnsEmpty_forAllProviders() {
        let providers = ["kimi", "qwen", "zhipu", "mimo", "deepseek", "openai", "ollama"]
        for id in providers {
    ...
    // MARK: - Kimi: tools with builtin_function/web_search

    @Test func kimi_searchEnabled_returnsBuiltinFunctionWebSearch() throws {
        let params = LLMSearchControl.extraBody(providerId: "kimi", baseURLHost: "api.moonshot.cn", searchEnabled: true)
        let tools = try #require(params["tools"] as? [[String: Any]])
        #expect(tools.count == 1)
        let tool = tools[0]
        #expect(tool["type"] as? String == "builtin_function")
    ...
    @Test func doubao_searchEnabled_returnsEmpty() {
        let params = LLMSearchControl.extraBody(providerId: "doubao", baseURLHost: "ark.cn-beijing.volces.com", searchEnabled: true)
        #expect(params.isEmpty)
    }

    // MARK: - Host-based fallback for custom endpoints

    @Test func custom_moonshotHost_resolvesToKimi() throws {
        let params = LLMSearchControl.extraBody(providerId: "custom", baseURLHost: "api.moonshot.cn", searchEnabled: true)
        let tools = try #require(params["tools"] as? [[String: Any]])
        #expect(tools[0]["type"] as? String == "builtin_function")
