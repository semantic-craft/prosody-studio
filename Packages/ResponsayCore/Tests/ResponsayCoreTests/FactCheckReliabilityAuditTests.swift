...
struct FactCheckReliabilityAuditTests {
    ...
    struct DeepLinkURLTests {
        ...
        @Test("国家法规库 — 无查询参数，仅打开首页")
        func govLaw_opensHomepage() {
            let a = VerificationAnchor(id: "a", label: "《民法典》第577条", kind: .law, query: "民法典 第577条")
            let route = router.route(for: a, source: .govLaw)
            #expect(route.kind == .deepLink)
            #expect(route.url?.host == "flk.npc.gov.cn")
            // No query parameter — user must search manually
            #expect(route.url?.query == nil)
        }

        @Test("北大法宝 — 无查询参数，仅打开首页")
        func pkulaw_opensHomepage() {
            ...
            #expect(route.url?.host == "www.pkulaw.com")
            #expect(route.url?.query == nil)
        }
    ...
    struct CNKIQueryTests {
        ...
        @Test("多词检索 → SU=('词1') AND SU=('词2')")
        func multiTerm() {
            let a = VerificationAnchor(id: "a", label: "比例原则", kind: .scholarlyArticle, query: "比例原则 行政法")
            let s = gen.strategy(for: a, scene: .academicWriting)
            #expect(s.cnkiQuery != nil)
            #expect(s.cnkiQuery!.expertQuery.contains("SU=('比例原则')"))
            #expect(s.cnkiQuery!.expertQuery.contains("AND"))
            #expect(s.cnkiQuery!.expertQuery.contains("SU=('行政法')"))
        }
