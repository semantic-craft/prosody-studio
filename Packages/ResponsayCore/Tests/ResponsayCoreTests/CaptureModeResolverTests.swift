...
@Test func modeResolver_resolvesLearningModes() {
    ...
    let analysis = CaptureModeResolver.resolve(.analysis)
    #expect(analysis.transformKind == .prosodyOnly)
    #expect(analysis.sidecarPolicy == .autoOpenProsody)
    #expect(analysis.insertPolicy == .noInsert)

    let practice = CaptureModeResolver.resolve(.practice)
    #expect(practice.transformKind == .practiceSeed)
    #expect(practice.sidecarPolicy == .autoOpenPractice)
    #expect(practice.insertPolicy == .noInsert)
}
