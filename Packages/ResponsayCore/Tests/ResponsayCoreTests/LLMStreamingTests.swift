...
struct LLMStreamOptionsControlTests {
    @Test func qwenAddsUsageStreamOptions_onlyOnDashScopeLane() {
        let qwen = LLMStreamOptionsControl.extraBody(providerId: "qwen", baseURLHost: "dashscope.aliyuncs.com")
        let streamOptions = qwen["stream_options"] as? [String: Bool]
...
struct StreamingTransformPromptBuilderTests {
    @Test func modePicksInstruction_outputIsRawText() {
        #expect(StreamingTransformPromptBuilder.build(mode: "rewrite", text: "x").system.contains("SAME language"))
        #expect(StreamingTransformPromptBuilder.build(mode: "translate", text: "x", targetLanguage: "de-DE").system.contains("German"))
        let polish = StreamingTransformPromptBuilder.build(mode: "polish", text: "uh hello")
        #expect(polish.user.contains("<raw_transcript>"))
        #expect(polish.user.contains("uh hello"))
        #expect(polish.system.contains("entire reply IS the inserted text"))
    }
}

@Suite(.serialized)
struct DirectStreamingTransformClientTests {
    ...
    @Test func streamsVendorDeltasThenDone() async throws {
        ...
        """
        let client = DirectStreamingTransformClient(endpoint: endpoint(), session: streamStubSession())
        let events = try await collect(client.stream(.init(text: "hello", mode: "polish")))
        #expect(events == [.delta("Hel"), .delta("lo"), .done])
    }

    @Test func surfacesHTTPErrorAsThrow() async throws {
        LLMStreamStubURLProtocol.status = 401
        LLMStreamStubURLProtocol.body = "unauthorized"
        let client = DirectStreamingTransformClient(endpoint: endpoint(), session: streamStubSession())
        await #expect(throws: LLMError.self) {
            _ = try await self.collect(client.stream(.init(text: "x", mode: "polish")))
        }
    }

    @Test func qwenStreamingRequestDefaultsToFastOfficialOpenAICompatibleShape() async throws {
        LLMStreamStubURLProtocol.status = 200
        LLMStreamStubURLProtocol.requestBody = Data()
        LLMStreamStubURLProtocol.body = "data: [DONE]\n\n"
        let client = DirectStreamingTransformClient(endpoint: qwenEndpoint(), session: streamStubSession())
        _ = try await collect(client.stream(.init(text: "hello", mode: "polish")))
    ...
    @Test func qwenStreamingRequestAllowsExplicitThinkingOn() async throws {
        LLMStreamStubURLProtocol.status = 200
        LLMStreamStubURLProtocol.requestBody = Data()
        LLMStreamStubURLProtocol.body = "data: [DONE]\n\n"
        let client = DirectStreamingTransformClient(endpoint: qwenEndpoint(thinking: true), session: streamStubSession())
        _ = try await collect(client.stream(.init(text: "hello", mode: "polish")))
