...
struct RewritePromptBuilderTests {
    @Test func rewrite_user_carriesTextAndAsksSameLanguage() {
        ...
        #expect(p.system.contains("THE SAME language"))
...
struct LLMThinkingControlTests {
    ...
    @Test func deepseek_minimax_mimo_zhipu_thinkingType() {
        #expect((body("deepseek", "deepseek-chat", true)["thinking"] as? [String: String])?["type"] == "enabled")
        #expect((body("minimax", "MiniMax-Text-01", true)["thinking"] as? [String: String])?["type"] == "adaptive")
    ...
    @Test func gemini_reasoningEffortNoneWhenOff() {
        ...
        // Pro/3-class models can't fully disable thinking — emit nothing rather than an invalid "none"
